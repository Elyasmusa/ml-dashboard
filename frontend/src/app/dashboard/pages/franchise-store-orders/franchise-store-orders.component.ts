...
@Component({
  selector: 'app-franchise-store-orders',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './franchise-store-orders.component.html',
  styleUrl: './franchise-store-orders.component.scss',
})
export class FranchiseStoreOrdersComponent implements OnInit {
  ...
  loadOrders(): void {
    ...
    this.inflowService.listFranchiseStoreOrders().subscribe({
      next: (response) => {
        this.salesOrders = this.sortOrdersByDate(response.data || []);
        this.loading = false;
        console.log(`Loaded ${this.salesOrders.length} franchise store orders`);
      },
      error: (err) => {
        console.error('Error loading franchise store orders:', err);
        this.error = `Failed to load franchise store orders: ${err.message || err.statusText || 'Unknown error'}`;
        this.loading = false;
      },
    });
  }
