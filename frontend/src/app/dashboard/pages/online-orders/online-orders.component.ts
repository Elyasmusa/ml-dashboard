...
export class OnlineOrdersComponent implements OnInit {
  ...
  loadOrders(): void {
    ...
    this.inflowService.listOnlineOrders().subscribe({
      next: (response) => {
        this.salesOrders = this.sortOrdersByDate(response.data || []);
        this.loading = false;
        console.log(`Loaded ${this.salesOrders.length} online orders`);
      },
      error: (err) => {
        console.error('Error loading online orders:', err);
        this.error = `Failed to load online orders: ${err.message || err.statusText || 'Unknown error'}`;
        this.loading = false;
      },
    });
  }
